<script>
    ...
    // action to register each card node
    ...
    function pileBounds(pileNo){
      ...
      const arr = get(cycle3array);              // snapshot store
    ...
    $: get(cycle3array), invalidate();
    $: if ($annotations) invalidate();
    ...
    {#if $statuscard}
      ...
      <div class="stage" bind:this={board}>
        <div class="container" bind:this={containerEl}>
          {#each $cycle3array as card (card.index)}
            <div
              class="card {card.selected ? 'selected' : ''} {card.cycle3pos >= 9 && card.cycle3pos <= 17 && !card.selected ? 'cycle-range' : ''}"
              use:collect={card}

    ...
    {#if $statuscard}
      ...
      <div class="stage" bind:this={board}>
        <div class="container" bind:this={containerEl}>
          {#each $cycle2array as card (card.index)}
            <div
              class="card {card.selected ? 'selected' : ''} {card.cycle2pos >= 9 && card.cycle2pos <= 17 && !card.selected ? 'cycle-range' : ''}"
              use:collect={card}
